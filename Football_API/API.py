...
images = first_match.findAll('img')[0]['src']  # รูปทีม
data["contents"][0]["hero"]["url"] = images
predict_url = "https://ball7m.com/" + first_match.findAll('a')[0]['href']
sleep(1)
...
data["contents"][0]["body"]["contents"][0]["contents"][0]["contents"][0]["text"] = TeamA
data["contents"][0]["body"]["contents"][0]["contents"][0]["contents"][1]["text"] = TeamB
...
for predict in All_predict:
    if "แนวโน้มเกม" in predict.text:
        data["contents"][0]["body"]["contents"][2]["contents"][0]["text"] = predict.text.strip(
        ).split("แนวโน้มเกม", 1)[1]
...
for match_all in soup.find_all("div", {"class": "col-6"}):
    ...
    data["contents"][count]["body"]["contents"][0]["contents"][0]["contents"][1]["text"] = TeamB

    print(str(TeamA) + " vs " + str(TeamB))
    All_predict = soup2.findAll("div", {"class": "card-body"})
    for predict in All_predict:
        if "แนวโน้มเกม" in predict.text:
            data["contents"][count]["body"]["contents"][2]["contents"][0]["text"] = predict.text.strip(
            ).split("แนวโน้มเกม", 1)[1]
    count += 1
